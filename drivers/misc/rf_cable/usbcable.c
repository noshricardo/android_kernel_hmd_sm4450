...
static const struct of_device_id usbcable_of_match[] = {
	{ .compatible = "qcom,usb_cable_eint", },
	{},
};
...
static int usbcable_probe(struct platform_device *pdev){
    ...
    if (usbcable_eint_num1 < 0) {
        ...
        ret = request_irq(usbcable_eint_num1, usbcable_irq_handler1,IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING , "USB_CABLE1", NULL);
        if (ret) {
    ...
    if (usbcable_eint_num2 < 0) {
        ...
        ret = request_irq(usbcable_eint_num2, usbcable_irq_handler2,IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING , "USB_CABLE2", NULL);
        if (ret) {
