						...
						void __user *argp)
{
	...
	if ((req.ifd_data_fd <= 0) || (req.virt_sb_base == NULL) ||
					(req.sb_len == 0)) {
		pr_err("Invalid input(s)ion_fd(%d), sb_len(%d), vaddr(0x)\n",
		...
		struct qseecom_client_send_service_ireq *send_svc_ireq_ptr)
{
	...
	if ((req_ptr == NULL) || (send_svc_ireq_ptr == NULL)) {
		pr_err("Error with pointer: req_ptr = , send_svc_ptr = \n");
		return -EINVAL;
	}
		...
		if (sg_ptr->nents == 1) {
			...
			if ((data->type == QSEECOM_CLIENT_APP &&
				...
				if ((qseecom.qsee_version >= QSEE_VERSION_40) &&
					...
					pr_err("App %s sgl PA exceeds 4G: phy_addr=ad, len=%x\n",
						data->client.app_name,
						sg->length);
					goto err;
				}
			...
			uint32_t sbuf_len, void *resp_buf, uint32_t rbuf_len)
{
	...
	pr_debug("sending cmd_req->rsp size: %u, ptr: 0x\n",
			req.resp_len);
	return ret;
}
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case QSEECOM_IOCTL_SET_MEM_PARAM_REQ: {
		...
		pr_debug("SET_MEM_PARAM: qseecom addr = 0x\n");
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
	...
	case QSEECOM_IOCTL_LOAD_APP_REQ: {
		...
		data->type = QSEECOM_CLIENT_APP;
		pr_debug("LOAD_APP_REQ: qseecom_addr = 0x\n");
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
	...
	case QSEECOM_IOCTL_UNLOAD_APP_REQ: {
		...
		pr_debug("UNLOAD_APP: qseecom_addr = 0x\n");
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		ret = qseecom_unload_app(data, false);
		atomic_dec(&data->ioctl_count);
	...
	case QSEECOM_IOCTL_APP_LOADED_QUERY_REQ: {
		...
		mutex_lock(&app_access_lock);
		atomic_inc(&data->ioctl_count);
		pr_debug("APP_LOAD_QUERY: qseecom_addr = 0x\n");
...
static int qseecom_release(struct inode *inode, struct file *file)
{
	...
	if (!data->released) {
		pr_debug("data: released=false, type=%d, mode=%d, data=0x\n",
			data->type, data->mode);
		switch (data->type) {
