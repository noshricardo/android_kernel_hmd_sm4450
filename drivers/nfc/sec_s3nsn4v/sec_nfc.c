...
#endif
//Added by jiangjiang.yang for NHK-1996 on 2024-04-08 begin
static void sec_nfc_set_mode(struct sec_nfc_info *info, enum sec_nfc_mode mode);
//Added by jiangjiang.yang for NHK-1996 on 2024-04-08 end
...
static int sec_nfc_chip_detect(struct sec_nfc_info *info)
{
    ...
    if (ret != sizeof(rx_buff)) {
        dev_dbg(info->dev, "failed to recv bootloader info: %d\n",ret);
        printk("failed to recv bootloader info: %d\n",ret);
        ret = -1;
        goto detect_end;
    }
    if (rx_buff[0] != 0x81 ||
                ...
                rx_buff[4] != 0x85))
    {
        dev_dbg(info->dev, "bootloader info is not correct!\n");
        printk("bootloader info is not correct!\n");
        ret = -1;
        goto detect_end;
    }
