...
/**
*@brief: get bootloader status from cmdline.
*@Return: BOOLEAN bootloader status.
*        TRUE : bootloader is unlocked.
*        FALSE: bootloader is locked.
*/
static bool is_bootloader_unlock(void)
{
	struct device_node *np;
	const char *bootparams;
	...
	if (!bootparams)
		pr_err("%s: failed to get bootargs property\n", __func__);
	else if (strnstr(bootparams, "ontimboot.bootloaderstate=unlocked",strlen(bootparams)))
		return true;
			   ...
			   struct qcom_mdt_metadata *mdata)
{
	...
	if (pas_init) {
		...
		if (ret) {
			...
			dev_err(dev, "%s : is_bootloader_unlock: %d\n",__func__, is_bootloader_unlock());
			if (!strcmp(fw_name, "modem.mdt") && 
			    (true == get_efuse_status())/* && 
			    (false == is_bootloader_unlock())*/){
				dev_err(dev, "Modem image is unsigned or corrupt, now reboot to red screen!\n");
