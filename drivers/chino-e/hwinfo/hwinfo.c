...
static void get_rfgpio_state(void)
{
	int val = 0;
        ...
        val = gpio_get_value(458);
        printk(KERN_INFO "%s: gapio83 val = %d\n", __func__, val);
        str[2] = val ? '1' : '0';

        val = gpio_get_value(459);
        printk(KERN_INFO "%s: gapio84 val = %d\n", __func__, val);
        str[1] = val ? '1' : '0';

        val = gpio_get_value(466);
        printk(KERN_INFO "%s: gapio91 val = %d\n", __func__, val);
        str[0] = val ? '1' : '0';

	memset(hwinfo[RF_GPIO].hwinfo_buf, 0x00, sizeof(hwinfo[RF_GPIO].hwinfo_buf));
        strncpy(hwinfo[RF_GPIO].hwinfo_buf, str, 3);
}

static void get_cablegpio_state(void)
{
        int ret = 0;
        ret = gpio_get_value(466);
        printk(KERN_INFO "cable_gpio=%d\n", ret);
...
{
	int i;
	unsigned int skuid_gpios[] = { 33, 32, 31 }; // from high to low
	int pin_val = 0;
	...
	for (i = 0; i < ARRAY_SIZE(skuid_gpios); i++)
		pin_val += !!gpio_get_value(skuid_gpios[i] + base_gpio) << 1;
	pin_val++; // SKU started from 1
	printk(KERN_INFO "%s: hw_sku=%d\n",__func__, pin_val);
