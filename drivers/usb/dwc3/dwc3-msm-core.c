...
static const char * const usb_ports_strings[] = {
	"TYPEC",
	"SMARTPIN",
	"USB_CURR"
};
...
static inline const char *usb_ports_string(enum usb_ports port)
{
	if (port < ARRAY_SIZE(usb_ports_strings))
		return usb_ports_strings[port];
...
static enum usb_role smartpin_role = USB_ROLE_NONE;

static int dwc3_msm_set_role(struct dwc3_msm *mdwc, enum usb_role role, enum usb_ports port)
{
	enum usb_role cur_role;

	static enum usb_role port_role[3] = {USB_ROLE_NONE};

	port_role[port] = role;
	smartpin_role = port_role[USB_SMARTPIN];

	dev_err(mdwc->dev, "%s: port = %s; role = %s; port_role[USB_TYPEC] = %s; port_role[USB_SMARTPIN] = %s;\n", __func__,
		usb_ports_string(port), usb_role_string(role), usb_role_string(port_role[USB_TYPEC]), usb_role_string(port_role[USB_SMARTPIN]));
	...
	dev_err(mdwc->dev, "%s: cur_role:%s \n", __func__, usb_role_string(cur_role));

	if((port == USB_SMARTPIN) && (port_role[USB_TYPEC] != USB_ROLE_NONE) && (cur_role == port_role[USB_TYPEC])){
		dev_err(mdwc->dev, "%s: --------------\n", __func__);
		mutex_unlock(&mdwc->role_switch_mutex);
		return 0;
	}
	else if(port == USB_CURR){
		if(port_role[USB_TYPEC] != USB_ROLE_NONE){
			port_role[USB_TYPEC] = role;
		}
		else if(port_role[USB_SMARTPIN] != USB_ROLE_NONE){
			port_role[USB_SMARTPIN] = role;
		}
	}
	...
	if (mdwc->drd_state != DRD_STATE_UNDEFINED && cur_role == role) {
		...
		mutex_unlock(&mdwc->role_switch_mutex);
		if((port == USB_TYPEC) && (role == USB_ROLE_HOST) && (port_role[USB_SMARTPIN] == USB_ROLE_HOST)){
			dwc3_ext_event_notify(mdwc);
			pr_err("hzn: dwc3_ext_event_notify-----------\n");
		}
	...
	pr_err("hzn: new_role:%s refcnt:%d-----------\n",
		usb_role_string(role), mdwc->refcnt_dp_usb);
	mutex_unlock(&mdwc->role_switch_mutex);
...
int smartpin_dwc3_msm_set_role(enum usb_role role)
{
	...
	if(__mdwc != NULL){
		ret = dwc3_msm_set_role(__mdwc, role, USB_SMARTPIN);
		if (ret < 0)
			return ret;
	}
	else
...
EXPORT_SYMBOL(smartpin_dwc3_msm_set_role);
...
static int dwc3_msm_usb_role_switch_set_role(struct usb_role_switch *sw, enum usb_role role)
{
	...
	return dwc3_msm_set_role(mdwc, role, USB_TYPEC);
}
#else
static int dwc3_msm_usb_role_switch_set_role(struct usb_role_switch *sw, enum usb_role role)
{
	...
	val = dwc3_msm_set_role(mdwc, role, USB_TYPEC);

	if((smartpin_role == USB_ROLE_HOST) && (role == USB_ROLE_NONE)){
		dev_err(mdwc->dev, "%s: typec_role = %s; smartpin_role = %s;\n", __func__,
				usb_role_string(role), usb_role_string(smartpin_role));
		msleep(1000);
		dwc3_msm_set_role(mdwc, smartpin_role, USB_SMARTPIN);
	}
		...
		const char *buf, size_t count)
{
	...
	ret = dwc3_msm_set_role(mdwc, role, USB_CURR);
	if (ret < 0)
		return ret;
...
static void dwc3_otg_sm_work(struct work_struct *w)
{
	...
	dev_err(mdwc->dev, "dwc3_otg_sm_work: %s state\n", state);
