...
static irqreturn_t pm8941_pwrkey_irq(int irq, void *_data)
{
	...
	#else
		pr_info("PMIC input: KPDPWR status=0x%02x, KPDPWR_ON=%d\n",
			sts, (sts & PON_KPDPWR_N_SET));
	#endif
	// ning.wei@hmd++ end
				...
				struct device_attribute *attr, char *buf)
{
	...
	*/
	...
	regmap_write(pwrkey->regmap, 0x800 + 0x44 , 0);
	regmap_write(pwrkey->regmap, 0x800 + 0x45 , 0);
	regmap_write(pwrkey->regmap, 0x800 + 0x46 , 1);
	regmap_write(pwrkey->regmap, 0x800 + 0x47 , 0x80);

	return scnprintf(buf, PAGE_SIZE, "You can press volume minus key to trigger ramdump now ! baseaddr=0x%x\n", pwrkey->baseaddr);
}

static ssize_t volume_minus_trigger_ramdump_store(struct device *dev,
				struct device_attribute *attr, const char *buf, size_t len)
{
	struct pm8941_pwrkey *pwrkey = dev_get_drvdata(dev);

	if (buf[0] == '0') {
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x44 , 0xf);
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x45 , 0x7);
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x46 , 0x4);
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x47 , 0x0);
	}
	else {
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x44 , 0);
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x45 , 0);
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x46 , 1);
		regmap_write(pwrkey->regmap, pwrkey->baseaddr + 0x47 , 0x80);
	}

	return len;
}
static DEVICE_ATTR_RW(volume_minus_trigger_ramdump);
// ning.wei@hmd++ end
...
static int pm8941_pwrkey_probe(struct platform_device *pdev)
{
	...
	// ning.wei@hmd++
	error = device_create_file(&pdev->dev, &dev_attr_volume_minus_trigger_ramdump);
	if (error) {
		pr_err("sysfs vol- trigger dump file creation failed, error=%d\n",
			error);
	}
	else {
		dev_set_drvdata(&pdev->dev, pwrkey);
		pr_err("sysfs vol- trigger dump file creat\n");
	}
	// ning.wei@hmd++
