...
static ssize_t show_tp_gesture_en(struct device *dev,struct device_attribute *attr,char *buf)
{
    int count = 0;
    u8 val = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
...
static ssize_t store_tp_gesture_en(struct device *dev,struct device_attribute *attr,const char *buf,size_t count)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    mutex_lock(&ts_data->input_dev->mutex);
    if (FTS_SYSFS_ECHO_ON(buf)) {
...
static DEVICE_ATTR(gesture_en,0664,show_tp_gesture_en,store_tp_gesture_en);
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    if(ret)
    {
        FTS_ERROR("failed to register platform device\n");
        return -ENODEV;
    }
    ret = device_create_file(&(gesture_device.dev),&dev_attr_gesture_en);
    dev_set_drvdata(&gesture_device.dev, ts_data);
