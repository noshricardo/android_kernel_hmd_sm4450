...
static int fts_ts_probe(struct spi_device *spi)
{
    ...
    FTS_ERROR("Touch Screen(SPI-2 BUS) driver prboe...");
    spi->mode = SPI_MODE_0;
    spi->bits_per_word = 8;
    ret = spi_setup(spi);
    if (ret < 0) {
    ...
    FTS_ERROR("Touch Screen(SPI BUS) driver prboe successfully");
    return 0;
}
...
const int chg_get_cmd(void)
{
    ...
    if (of_chosen) {
        ...
        if (!bootargs)
            FTS_INFO("failed to bootargs");
        else {
            FTS_INFO("bootargs=: %s", bootargs);
        }
    } else
        FTS_INFO("failed to get /chosen");
    if (strnstr(bootargs,"ontimboot.mode",strlen(bootargs)) != NULL) {
        if (strnstr(bootargs,"charger",strlen(bootargs)) != NULL) {
            FTS_INFO("charger mode");
            return 1;
        }
    } else
        FTS_INFO("narmal boot");
    return 0;
}
add by zsh 2024/3/28/end */
static int __init fts_ts_spi_init(void)
{
   ...
   // int ret = -1;
    //msleep(6000);
    printk(KERN_ERR "focaltech get panel is %s\n",panel_name_find);
    if(strcmp(panel_name_find,"mdss_dsi_ft8057_fhd_plus_90_video") != 0){
        return 0;
    }
/*
    ret = chg_get_cmd();
