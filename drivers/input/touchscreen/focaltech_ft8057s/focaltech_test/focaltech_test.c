...
static ssize_t fts_test_show_1(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t fts_test_store_1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
...
static DEVICE_ATTR(fts_test, S_IRUGO | S_IWUSR, fts_test_show_1, fts_test_store_1);
    ...
    struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    memset(fwname, 0, sizeof(fwname));
    snprintf(fwname, FILE_NAME_LENGTH, "%s", "focaltech_ft8057s.ini");
    ...
    if (fts_ftest->result == TEST_RESULT_INIT) {
        FTS_ERROR("=======Tp test %s.\n", "has not started");
	result_buffer = "Tp test has not started.\n";
    } else if (fts_ftest->result == TEST_RESULT_IN_PROGRESS) {
        FTS_ERROR("=======Tp test %s.\n", "is in progress");
	result_buffer = "Tp test is in progress.\n";
    } else if (fts_ftest->result == TEST_RESULT_PASS) {
        FTS_ERROR("=======Tp test %s.\n", "succeeds");
	result_buffer = "Tp test pass.\n";
    } else {
        FTS_ERROR("=======Tp test %s.\n", "fails");
	result_buffer = "Tp test failure.\n";
    }

    datalen = strlen(result_buffer);
    if(count > datalen - *ppos) {
         count = datalen - *ppos;
    }
    if (copy_to_user(buf, result_buffer, count)) {
...
int fts_test_init(struct fts_ts_data *ts_data)
{
	...
	return -ENOMEM;
        ...
        proc_create_data("tp_test", 0664, proc_tp_test, &tp_test_fops, NULL);
