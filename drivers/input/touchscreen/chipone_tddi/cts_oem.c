...
#define OEM_OF_DEF_PROPVAL_RAWDATA_MIN      1100
#define OEM_OF_DEF_PROPVAL_RAWDATA_MAX      3000
#define OEM_OF_DEF_PROPVAL_NOISE_FRAMES     16
#define OEM_OF_DEF_PROPVAL_NOISE_MAX        125
#define OEM_OF_DEF_PROPVAL_OPEN_MIN         1300
#define OEM_OF_DEF_PROPVAL_SHORT_MIN        1300
...
#define OEM_SELFTEST_PROC_FILENAME          "tp_test"
...
int cts_oem_init(struct chipone_ts_data *cts_data)
{
    ...
    cts_info(" - Create '/proc/touchscreen"OEM_SELFTEST_PROC_FILENAME"'");
    /*add node begin*/
    proc_tp_test = proc_mkdir("touchscreen", NULL);
    if (!proc_tp_test) {
        cts_err("procfs(test) create fail");
        remove_proc_subtree("proc_tp_test", NULL);
	    return -ENOMEM;
    } else {
         oem_data->selftest_proc_entry =
             proc_create_data(OEM_SELFTEST_PROC_FILENAME,
                      S_IRUGO, proc_tp_test, &selftest_proc_fops, cts_data);
          if (oem_data->selftest_proc_entry == NULL) {
              cts_err("Create '/proc/touchscreen"OEM_SELFTEST_PROC_FILENAME"' failed");
              ret = -EFAULT;
              goto free_oem_data;
          }
    }
    ...
    oem_data->limit_entry =
        proc_create_data(OEM_LIMIT_PROC_FILENAME,
            S_IRUGO, proc_tp_test, &cts_limit_ops, cts_data);
    if (oem_data->limit_entry == NULL) {
        cts_err("Create '/proc/touchscreen"OEM_LIMIT_PROC_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }
    ...
    oem_data->factory_test_entry =
        proc_create_data(OEM_FACTORY_TEST_PROC_FILENAME,
            S_IRUGO,proc_tp_test, &cts_factory_test_ops, cts_data);
    if (oem_data->factory_test_entry == NULL) {
        cts_err("Create '/proc/touchscreen"OEM_FACTORY_TEST_PROC_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }

    oem_data->rawdata_proc_entry = proc_create_data(OEM_RAWDATA_PROC_FILENAME,
            S_IRUGO, proc_tp_test, &cts_rawdata_ops, cts_data);
    if (oem_data->rawdata_proc_entry == NULL) {
        cts_err("Create '/proc/touchscreen"OEM_RAWDATA_PROC_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }

    oem_data->manual_proc_entry = proc_create_data(OEM_MANUAL_PROC_FILENAME,
            S_IRUGO, proc_tp_test, &cts_manual_ops, cts_data);
    if (oem_data->manual_proc_entry == NULL) {
        cts_err("Create '/proc/touchscreen"OEM_MANUAL_PROC_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }

    oem_data->diffdata_proc_entry = proc_create_data(OEM_DIFFDATA_PROC_FILENAME,
            S_IRUGO, proc_tp_test, &cts_diffdata_ops, cts_data);
    if (oem_data->diffdata_proc_entry == NULL) {
        cts_err("Create '/proc/touchscreen"OEM_DIFFDATA_PROC_FILENAME"' failed");
        ret = -EFAULT;
        goto free_oem_data;
    }
