...
static int cts_init_fwdata(struct cts_device *cts_dev)
{
    ...
    snprintf(touch_version, sizeof(touch_version),"chipone-tddi ver 0x%x",fwdata->version);
...
int cts_irq_handler(struct cts_device *cts_dev)
{
    ...
    if ((cts_data->pm_suspend)) {
        ret = wait_for_completion_timeout(
                  ...
                  msecs_to_jiffies(CTS_TIMEOUT_COMERR_PM));
        if (!ret) {
...
void cts_log(int level, const char *fmt, ...)
{
    ...
    } else if (level < CTS_DRIVER_LOG_DEBUG || cts_show_debug_log) {
        // vprintk(fmt, args);
		char buf[512];
