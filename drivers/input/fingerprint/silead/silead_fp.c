...
silfp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case SIFP_IOC_PROC_NODE:
        ...
        if (arg) {
            ...
            LOG_MSG_DEBUG(INFO_LOG, "vendor_name = %s\n", vendor_name);
            memset(fp_info,0,sizeof(fp_info));
            if (NULL != strstr(vendor_name, "syx"))
                snprintf(fp_info, sizeof(fp_info),"%s","GSL6193LMSCNB_SYX");
            else if (NULL != strstr(vendor_name, "sa"))
                snprintf(fp_info, sizeof(fp_info),"%s","GSL6193LMSCNB_SA");
            else
                snprintf(fp_info, sizeof(fp_info),"%s","GSL6193LMSCNB");
        }
#endif /* PROC_NODE */
#ifdef CLASS_NODE
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    LOG_MSG_DEBUG(ERR_LOG, "silfp_probe success\n");
    return status;
